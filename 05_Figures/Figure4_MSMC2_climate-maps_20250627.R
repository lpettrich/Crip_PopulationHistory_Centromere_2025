...
# Set directory
setwd("~/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/")
getwd()
...
# Read in data
mf <- read.table("~/sciebo/RecombinationLandscape_CRIP/01_Revison/Sorted_final_files/Final_Supplement/Zenodo/MSMC2/Mean-values-msmc2-MF.csv", header = TRUE, sep = ";")
mg <- read.table("~/sciebo/RecombinationLandscape_CRIP/01_Revison/Sorted_final_files/Final_Supplement/Zenodo/MSMC2/Mean-values-msmc2-MG.csv", header = TRUE, sep = ";")
nmf <- read.table("~/sciebo/RecombinationLandscape_CRIP/01_Revison/Sorted_final_files/Final_Supplement/Zenodo/MSMC2/Mean-values-msmc2-NMF.csv", header = TRUE, sep = ";")
si <- read.table("~/sciebo/RecombinationLandscape_CRIP/01_Revison/Sorted_final_files/Final_Supplement/Zenodo/MSMC2/Mean-values-msmc2-SI.csv", header = TRUE, sep = ";")
ss <- read.table("~/sciebo/RecombinationLandscape_CRIP/01_Revison/Sorted_final_files/Final_Supplement/Zenodo/MSMC2/Mean-values-msmc2-SS.csv", header = TRUE, sep = ";")
...
mdata <- mdata %>% 
  mutate(Population = case_when(
    Population == "MG" ~ "Hesse (MG)",
    Population == "NMF" ~ "Lorraine (NMF)",
    Population == "MF" ~ "Rhône-Alpes (MF)",
    Population == "SI" ~ "Piedmont (SI)",
    Population == "SS" ~ "Andalusia (SS)",
    TRUE ~ Population
  )
  )

goodPopOrder <- c("Hesse (MG)", "Lorraine (NMF)", "Rhône-Alpes (MF)", "Piedmont (SI)", "Andalusia (SS)")
mdata$Population <- factor(mdata$Population, levels = goodPopOrder)
...
points(samples, pch = 21, cex = 2, col = "black", bg = my_palette)
...
save_plot("/home/alle/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/MSMC2-climate-maps-CRIP-Years-combined_Manuscript_20250614.svg", 
          all_plots, nrow = 7, ncol = 4, base_asp = 1.7, dpi = 500, bg = "white", scale = 0.57)

save_plot("/home/alle/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/MSMC2-climate-maps-CRIP-Years-combined_Manuscript_20250614.png", 
          all_plots, nrow = 7, ncol = 4, base_asp = 1.7, dpi = 500, bg = "white", scale = 0.57)

# SAVE SEPERATE AND MERGE WITH INKSCAPE
pcol
save_plot("/home/alle/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/MSMC2-CRIP-Years_Manuscript_20250614.svg", 
          pcol, nrow = 3, ncol = 2, base_asp = 1.45, dpi = 500, bg = "white", scale = 1)



bottom_row
save_plot("/home/alle/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/Cimate-Maps_Manuscript_20250614.svg", 
          bottom_row, nrow = 2, ncol = 3, base_asp = 1.9, dpi = 500, bg = "white", scale = 0.8)

# make legend with squares
pNeYears + geom_point(shape = 15)

ggp2 + geom_point(shape = 15)

ggp <- ggplot(mdata_ne, aes(x = mean_years, y = mean_Ne, colour = Population)) + 
  geom_point(size = 5, shape = 15)  +
  scale_x_continuous(limits=c(10^2,10^6), trans="log10", labels = scientific_10) +
  scale_y_continuous(labels = comma) +
  scale_color_manual(values = my_palette_ne)  +
  labs(x = "Years ago", y = "Effective \npopulation size") + 
  theme(axis.text.x = element_text(size = 14, color = "black"),
        axis.text.y = element_text(size = 14, color = "black"),
        axis.title.y = element_text(size = 14,face = "bold", color = "black"),
...
ggp2 <- ggplot(mdata, aes(x = years, y = rel.cc, colour = Population)) + 
  ...
  scale_x_continuous(limits=c(10^2,10^6), trans="log10", labels = scientific_10) +
  scale_y_continuous(limits = c(0,1.2), labels = comma, breaks = c(0, 0.5, 1)) +
  scale_color_manual(values = my_palette_rccr)  +
  labs(x = "Years ago", y = "Relative \ncross-coalescence \nrate") + 
  theme(axis.text.x = element_text(size = 14, color = "black"),
        axis.text.y = element_text(size = 14, color = "black"),
        axis.title.y = element_text(size = 14,face = "bold", color = "black"),
...
pcol
save_plot("/home/alle/sciebo/RecombinationLandscape_CRIP/01_Revison/NewResults/Plots/MSMC2-CRIP-Years_Manuscript_LEGEND_20250614.svg", 
          pcol, nrow = 3, ncol = 2, base_asp = 1.45, dpi = 500, bg = "white", scale = 1)
